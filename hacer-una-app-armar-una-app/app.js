...
function saveCloudData() {
  if (!supabaseClient || !currentAuthUserId || !currentUser || !state.users[currentUser]) return;
...
function renderOverview() {
  ...
  const goals = activeOrFutureGoals(baseKey);
  ...
  const activeCount = goals.filter((goal) => goalAppliesToMonth(goal, baseKey)).length;
  const futureCount = goals.filter((goal) => baseKey < goalStartMonth(goal)).length;
  ...
  if (!data.goals.length) {
    ...
    els.overviewProjection.textContent = "Todas las metas cargadas ya terminaron según sus fechas. Podés crear una nueva o revisar el historial mensual.";
  } else {
    ...
    const scope = `${activeCount} meta${activeCount === 1 ? "" : "s"} activa${activeCount === 1 ? "" : "s"} este mes${futureCount ? ` y ${futureCount} futura${futureCount === 1 ? "" : "s"}` : ""}.`;
...
function renderAi() {
  ...
  const phrases = [
    "La constancia gana por acumulación: hoy ajustás un poco, mañana tu meta respira mejor.",
    "Buen mes para mirar los gastos chicos: juntos suelen hacer ruido de billetera grande.",
    "Si el número no acompaña, no es fracaso: es información para recalcular con inteligencia.",
    "Cada aporte baja el ahorro mensual objetivo. Pequeño movimiento, gran efecto en la meta.",
  ];

  if (available < 500000) {
    els.aiTitle.textContent = "Atención con el disponible";
    els.aiMessage.textContent = "La IA te diría que este mes conviene ajustarse: el disponible quedó por debajo de $ 500.000 después de contemplar tus metas.";
  } else {
    els.aiTitle.textContent = "Coach IA";
    els.aiMessage.textContent = phrases[(new Date(`${selectedMonth()}-01`).getMonth() + aiNudge) % phrases.length];
  }
