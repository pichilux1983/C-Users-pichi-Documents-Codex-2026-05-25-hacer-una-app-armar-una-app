...
function renderGoals() {
  ...
  activeGoals.forEach((goal) => {
    ...
    if (savedThisMonth > 0) {
      const contributionDetails = node.querySelector(".contribution-details");
      const currentContribution = document.createElement("div");
      currentContribution.className = "current-contribution";
      currentContribution.innerHTML = `
        <div>
          <span>Aporte cargado este mes</span>
          <strong>${pesos.format(savedThisMonth)}</strong>
        </div>
        <div class="row-actions">
          <button class="edit-contribution" type="button" title="Editar aporte">✎</button>
          <button class="delete-contribution" type="button" title="Eliminar aporte">×</button>
        </div>
      `;
      currentContribution.querySelector(".edit-contribution").addEventListener("click", () => editGoalContribution(goal));
      currentContribution.querySelector(".delete-contribution").addEventListener("click", () => deleteGoalContribution(goal));
      contributionDetails.appendChild(currentContribution);
    }
...
function editGoalContribution(goal) {
  const month = ensureMonth();
  const currentAmount = month.contributions?.[goal.id] || 0;
  const amount = prompt("Aporte ahorrado de este mes", pesos.format(currentAmount));
  if (amount === null) return;
  const value = parseMoney(amount);

  month.contributions = month.contributions || {};
  if (value > 0) {
    month.contributions[goal.id] = value;
  } else {
    delete month.contributions[goal.id];
  }
  saveAndRender();
}

function deleteGoalContribution(goal) {
  if (!confirm("¿Eliminar el aporte de ahorro cargado para este mes?")) return;
  const month = ensureMonth();
  if (month.contributions) {
    delete month.contributions[goal.id];
  }
  saveAndRender();
}
